...
const ToDoList: FC<ToDoListType> = ({ toDo, setToDo }) => {
    ...
    const saveToDo = async (id: number, title: string) => {
        ...
        const respone = await axios.put(`https://61f29e642219930017f50783.mockapi.io/todos`, { title: title })
                        ...
                        {isEdit === item.id ? (
                            <div className={style.editBlock}>
                                <div className={style.editModal}>
                                    ...
                                    <div className={style.btnEditBlock}>
                                        <button className={style.saveBtn} onClick={() => saveToDo(item.id, item.title)}>Save</button>
